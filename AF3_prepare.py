...
Creates per job directories containing:
  - input.json (AlphaFold3 style by default; ONE JSON OBJECT per file)
  - job.sbatch (optional)
...
Still supports alphafoldserver output if you need it.
...
# Important: many runners are strict; version=1 is widely accepted.
# If your runner explicitly supports version 2, you can set DEFAULT_AF3_VERSION back to 2.
DEFAULT_AF3_VERSION = 1
...
def main() -> None:
    ...
    p.add_argument(
        ...
        action="append",
        default=[],
        help="Add small molecule ligand(s) by CCD code, optionally with count. Example: --ligand NAD or --ligand EDO:2",
    )

    p.add_argument("--af3-dialect", default=DEFAULT_AF3_DIALECT, help="AF3 dialect field (default alphafold3)")
    p.add_argument("--af3-version", type=int, default=DEFAULT_AF3_VERSION, help="AF3 version field (default 1)")
    ...
    def emit(job_name: str, obj: dict) -> None:
        ...
        # Critical fix:
        # alphafold3 expects a single JSON object per file (NOT a list wrapper).
